  ...
  const fetchDashboardData = useCallback(
    async (isInitial = false) => {
      try {
        ...
        const [enqRes, pkgRes, recentRes, healthRes] = await Promise.all([
        ...
        if (healthRes && healthRes.ok) {
          ...
          if (healthData) {
            setHealth({
              ...
              emailService: healthData.integrations?.emailProvider ? "Online" : "Not Configured",
              redis:
          ...
          setHealth({
            apiServer: healthRes ? "Degraded" : "Offline",
            database: "Offline",
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            ...
            <div className="space-y-3">
              {[
                { name: "API Server", status: health.apiServer },
                { name: "Database", status: health.database },
                { name: "Email Service", status: health.emailService },
                { name: "Redis Cache", status: health.redis },
              ].map((service) => {
                const isOnline = service.status === "Online";
                const isOffline = service.status === "Offline" || service.status === "Degraded";
                const isWarning =
                ...
                return (
                  <div key={service.name} className="flex items-center justify-between">
                    ...
                    <span className={`flex items-center gap-1.5 text-xs font-medium ${colorClass}`}>
                      <span
                        className={`w-2 h-2 rounded-full ${dotClass} ${isOnline ? "animate-pulse" : ""}`}
                      />
                      {service.status}
                    </span>
                  </div>
